...
def connect_database_MDCdata(ata, excl_eqid, airline_operator, include_current_message, from_dt, to_dt):
    ...
    if include_current_message == 0:    
        if ata == 'ALL' and excl_eqid == 'NONE':
            sql = "SELECT mg.AC_SN, mg.AC_TN, mg.FLIGHT_LEG, mg.ATA, mg.ATA_NAME, mg.LRU, mg.MSG_Date, mg.msg_status, mg.FLIGHT_PHASE, mg.MSG_TYPE, mg.INTERMITNT, mg.EQ_ID, mg.SOURCE, dd.LABEL, dd.LRU_NAME, SUBSTRING(mg.ATA, 0, CHARINDEX('-', mg.ATA)) AS ATA_Main, SUBSTRING(mg.ATA, 4,4) AS ATA_SUB, dd.MSG_ID FROM MDC_MSGS mg JOIN LRU_DIAG_DATA dd ON dd.MSG_ID = mg.MSG_ID WHERE SUBSTRING(mg.ATA, 0, CHARINDEX('-', mg.ATA)) IN " + str(all_ata_str) + " AND EQ_ID IN " + str(
                all_eqid_str) + " AND FLIGHT_PHASE IS NOT NULL AND INTERMITNT IS NOT NULL AND MSG_Date BETWEEN '" + from_dt + "' AND '" + to_dt + "'"
        elif excl_eqid == 'NONE':
            sql = "SELECT mg.AC_SN, mg.AC_TN, mg.FLIGHT_LEG, mg.ATA, mg.ATA_NAME, mg.LRU, mg.MSG_Date, mg.msg_status, mg.FLIGHT_PHASE, mg.MSG_TYPE, mg.INTERMITNT, mg.EQ_ID, mg.SOURCE, dd.LABEL, dd.LRU_NAME, SUBSTRING(mg.ATA, 0, CHARINDEX('-', mg.ATA)) AS ATA_Main, SUBSTRING(mg.ATA, 4,4) AS ATA_SUB, dd.MSG_ID FROM MDC_MSGS mg JOIN LRU_DIAG_DATA dd ON dd.MSG_ID = mg.MSG_ID WHERE SUBSTRING(mg.ATA, 0, CHARINDEX('-', mg.ATA)) IN " + str(ata) + " AND EQ_ID IN " + str(
                all_eqid_str) + " AND FLIGHT_PHASE IS NOT NULL AND INTERMITNT IS NOT NULL AND MSG_Date BETWEEN '" + from_dt + "' AND '" + to_dt + "'"
        elif ata == 'ALL':
            sql = "SELECT mg.AC_SN, mg.AC_TN, mg.FLIGHT_LEG, mg.ATA, mg.ATA_NAME, mg.LRU, mg.MSG_Date, mg.msg_status, mg.FLIGHT_PHASE, mg.MSG_TYPE, mg.INTERMITNT, mg.EQ_ID, mg.SOURCE, dd.LABEL, dd.LRU_NAME, SUBSTRING(mg.ATA, 0, CHARINDEX('-', mg.ATA)) AS ATA_Main, SUBSTRING(mg.ATA, 4,4) AS ATA_SUB, dd.MSG_ID FROM MDC_MSGS mg JOIN LRU_DIAG_DATA dd ON dd.MSG_ID = mg.MSG_ID WHERE SUBSTRING(mg.ATA, 0, CHARINDEX('-', mg.ATA)) IN " + str(all_ata_str) + " AND EQ_ID NOT IN " + str(
                excl_eqid) + " AND FLIGHT_PHASE IS NOT NULL AND INTERMITNT IS NOT NULL AND MSG_Date BETWEEN '" + from_dt + "' AND '" + to_dt + "'"
        else:
            sql = "SELECT mg.AC_SN, mg.AC_TN, mg.FLIGHT_LEG, mg.ATA, mg.ATA_NAME, mg.LRU, mg.MSG_Date, mg.msg_status, mg.FLIGHT_PHASE, mg.MSG_TYPE, mg.INTERMITNT, mg.EQ_ID, mg.SOURCE, dd.LABEL, dd.LRU_NAME, SUBSTRING(mg.ATA, 0, CHARINDEX('-', mg.ATA)) AS ATA_Main, SUBSTRING(mg.ATA, 4,4) AS ATA_SUB, dd.MSG_ID FROM MDC_MSGS mg JOIN LRU_DIAG_DATA dd ON dd.MSG_ID = mg.MSG_ID WHERE SUBSTRING(mg.ATA, 0, CHARINDEX('-', mg.ATA)) IN " + str(ata) + " AND EQ_ID NOT IN " + str(
                excl_eqid) + " AND FLIGHT_PHASE IS NOT NULL AND INTERMITNT IS NOT NULL AND MSG_Date BETWEEN '" + from_dt + "' AND '" + to_dt + "'"
