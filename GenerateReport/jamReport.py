...
def jamReport(OutputTableHistory, ACSN_chosen,MDCdataDF):
   ...
   datatofilter2 = datatofilter.set_index(["AC_SN", "Equation ID"]).sort_index().loc[
                           pd.IndexSlice[listoftuplesACID], :].reset_index()
   listoftuplesACFL = list(zip(datatofilter2["AC_SN"], datatofilter2["Flight Leg No"]))
   datatofilter3 = datatofilter.set_index(["AC_SN", "Flight Leg No"]).sort_index()
